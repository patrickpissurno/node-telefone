...
export default function parse(value: string, options: Options = defaultOptions): string {
  ...
  if (telefone.length === 12 || telefone.length === 13) {
    if (telefone.substring(0, 2) === '55') telefone = telefone.substring(2, telefone.length)
    else return null // DDI não brasileiro
  }
  ...
  if (options.apenasCelular === true && telefone.length === 10) return null // todos os celulares tem 11 dígitos

  if (options.apenasFixo === true && telefone.length === 11) return null // todos os telefones fixos tem 10 dígitos

  if (telefone.length === 11) {
    if (telefone.substring(2, 3) !== '9') return null // todos os telefones celulares começam com 9
    if (telefone.substring(3, 4) === '0') return null // não existe 90
  } else if (telefone.length === 10 && iniciaisFixo.indexOf(telefone.substring(2, 3)) === -1) return null // inicial do celular fixo

  const ddd = telefone.substring(0, 2)
  if (codigosDDD.indexOf(parseInt(ddd)) === -1) return null // código de DDD inválido

  if (options.apenasDDD != null && options.apenasDDD.indexOf(ddd) === -1) return null // código de DDD inválido
